...
function PitchModal({ session, onClose, onContact }: { session: Session; onClose: () => void; onContact: (type: MsgType) => void }) {
  ...
  const msg = `Opa ${nome}! Aqui é o Bruno Dramusse. Tenho apenas 5 vagas disponíveis para a Mentoria Individual. Li as tuas respostas no formulário e tens o perfil certo para estar aqui. Queres fechar uma das vagas? Se tiveres alguma dúvida antes de entrar marcamos uma call rápida para esclarecer, caso contrário fechamos direto agora.`;
        ...
        <div className="p-5">
          <div className="rounded-xl bg-white/[0.04] border border-white/10 p-4">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-medium text-white/50">Mensagem</span>
              <button onClick={copy} className="text-xs rounded-md px-2.5 py-1 bg-white/10 hover:bg-white/15 text-white/60 hover:text-white transition-colors">
            ...
            {phone && (
              <a
                ...
                className="mt-4 flex items-center justify-center gap-2 w-full rounded-lg bg-[#25D366] hover:opacity-90 text-white text-xs font-medium py-2.5 transition-opacity"
              >
